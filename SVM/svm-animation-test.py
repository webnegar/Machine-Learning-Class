...
decision_text = ax.text(0.02, 0.95, '', transform=ax.transAxes, fontsize=12)
click_text = ax.text(0.02, 0.90, '', transform=ax.transAxes, fontsize=12)

ax.set_xlabel("X1")
ax.set_ylabel("X2")
ax.set_title("SVM غیرخطی با کنترل پخش انیمیشن")
ax.legend()
ax.grid(True)
...
def animate(i):
    ...
    return [_contourf, *_contour_lines]
...
def on_click(event):
    if event.inaxes == ax and event.xdata is not None and event.ydata is not None:
        ...
        val = model.decision_function([[event.xdata, event.ydata]])[0]
        click_text.set_text(f"Clicked → class {pred}, val={val:.2f}")
        fig.canvas.draw_idle()
...
# --- دکمه‌ها ---
ax_play = plt.axes([0.25, 0.05, 0.15, 0.075])
ax_pause = plt.axes([0.45, 0.05, 0.15, 0.075])
ax_reset = plt.axes([0.65, 0.05, 0.15, 0.075])
